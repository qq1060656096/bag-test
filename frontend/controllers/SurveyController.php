    ...
    public function actionIndexAjax()
    {
        ...
        if ($pageCount > 0 && $page > 0) {
            ...
            foreach ($a_models as $key => $row) {
                ...
                $is_top_text = '';
                $is_top_url = '';
                $op0 = Yii::$app->urlManager->createUrl([
                    'survey/recommend',
                    'id' => $row->id,
                    'op' => 0
                ]);
                $op1 = Yii::$app->urlManager->createUrl([
                    'survey/recommend',
                    'id' => $row->id,
                    'op' => 1
                ]);
...
str;
                ...
                if ($self > 0) {
                    ...
                    // 发布
                    $row_ur_done_publish = Yii::$app->urlManager->createUrl([
                        'survey/done',
                        'is_ajax' => 1,
                        'id' => $row->id
                    ]);
                    $row_ur_change = Yii::$app->urlManager->createUrl([
                        'survey/step2',
                        'id' => $row->id
                    ]);
                    $data .= <<<str
...
//        exit;
     ...
     */
    public function actionIsNotPublish($id)
    {
        $this->layout = false;
        $viewData = array();
        $model = Survey::findOne($id);
